...
<template>
  <div
    ...
    <header class="elem-header flex flex-row justify-between">
      <div class="flex items-center gap-x-2 flex-wrap">
        <div class="flex items-center gap-x-1">
          ...
          <template v-if="props.type === 'deck'">
            <el-tag
              size="small"
              :type="props.ext === '.toml' ? 'success' : 'primary'"
              disable-transitions>
      ...
      <aside class="flex flex-wrap items-center justify-center gap-x-2">
        <span v-if="props.downloadNum" class="flex items-center text-xs text-blue-500">
          ...
          >{{
            props.downloadNum > 1000
              ? (props.downloadNum / 1000).toFixed(1) + 'k'
              : props.downloadNum
          }}
        </span>
        <span v-else />
    ...
    <footer
      class="py-1 border-t flex justify-between flex-row-reverse flex-wrap text-gray-500 text-xs italic">
      ...
      <div class="flex">
        ...
        <span class="ml-2" v-if="props.updateTime && props.updateTime !== 0"
          >最近更新于 {{ dayjs.unix(props.updateTime).format('YYYY-MM-DD HH:MM') }}</span
        >
      </div>
    </footer>
  </div>
